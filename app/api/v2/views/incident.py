...
from app.api.v2.common.validator import required
...
def get_incidents(type, identity):
    ...
    if isAdmin(identity):
        ...
        return jsonify({"error": "You are not authorized to view this record"})
...
def edit_redflag_status(identity, redflag_id):
    ...
    if isAdmin(identity):
        ...
        if data['type'] == 'intervention':
            return jsonify(
                {"errors": "You are trying to edit an intervention record, use /interventions/<int:intervention_id>/status endpoint instead", "status": 400}), 400

        if not data['status'].strip(' '):
            return jsonify(
                {"errors": "Red-flag status cannot be null", "status": 400}), 400
        ...
        return jsonify(
            {"errors": "You have no permissions to edit this record. Contact the administrator", "status": 400}), 400
...
def edit_intervention_status(identity, intervention_id):
    ...
    if isAdmin(identity):
        ...
        if data['type'] == 'red-flag':
            return jsonify(
                {"errors": "You are trying to edit a red-flag record, use /red-flags/<int:redflag_id>/status endpoint instead", "status": 400}), 400
